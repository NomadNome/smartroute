...
1. SAFETY (0-10): Based on crime incidents near stations
   - 9-10: Very safe (0-2 crimes in area)
   - 7-8: Safe (3-5 crimes in area)
   - 5-6: Moderate (6-10 crimes in area)
   - 3-4: Less safe (11-20 crimes in area)
   - 0-2: Avoid if possible (20+ crimes in area)

2. RELIABILITY (0-10): Based on on-time performance of subway lines
   - 9-10: Excellent (95%+ on-time)
   - 7-8: Good (90-95% on-time)
   - 5-6: Average (85-90% on-time)
   - 3-4: Poor (75-85% on-time)
   - 0-2: Very unreliable (<75% on-time)

3. EFFICIENCY (0-10): Based on number of transfers and total travel time
   - 10: Direct route (0 transfers)
   - 8-9: 1 transfer
   - 6-7: 2 transfers
   - 4-5: 3 transfers
   - 0-3: 4+ transfers
...
3. BalancedRoute - Best overall experience (balanced scores across all metrics)

For each route, provide:
- stations: List of station names in order (exact names)
- lines: List of transit line numbers/letters for each leg (e.g., ["1", "1", "A"] - one entry per leg/segment)
- estimated_time_minutes: Total travel time estimate in minutes (realistic)
- explanation: Clear explanation of why this route is best for its category
- safety_score: 0-10 integer score based on crime data provided
- reliability_score: 0-10 integer score based on line performance
- efficiency_score: 0-10 integer score based on transfers and time

IMPORTANT:
- Return scores as numbers (0-10), not decimals
- The lines array must have values for each segment between stations
- Always return valid JSON with no markdown code blocks or additional text"""
