...
# MTA Configuration
MTA_API_BASE = 'http://datamine.mta.info/api/get'
MTA_FEEDS = {
    '1': 'subway_1_2_3_4_5',
    '2': 'subway_6_7',
    '3': 'subway_a_c_e',
    ...
    try:
        ...
        try:
            secret = secretsmanager_client.get_secret_value(
                SecretId='smartroute/api-keys'
            )
            api_key = json.loads(secret['SecretString']).get('mta_api_key')
            if not api_key or api_key.startswith('PLACEHOLDER'):
                logger.error("MTA API key not configured in Secrets Manager")
                return {
                    'statusCode': 401,
                    'error': 'MTA API key not configured',
                    'message': 'Update smartroute/api-keys in Secrets Manager'
                }
        except Exception as e:
            logger.error(f"Failed to retrieve API key: {str(e)}")
            return {
                'statusCode': 401,
                'error': f'Secret retrieval failed: {str(e)}'
            }

        # Fetch and process data from all MTA subway feeds
        all_vehicles = []
        all_alerts = []
        total_bytes = 0

        for feed_id, feed_name in MTA_FEEDS.items():
            try:
                logger.info(f"Fetching feed {feed_id}: {feed_name}")

                # Fetch protobuf from MTA
                vehicles, alerts, raw_bytes = fetch_and_decode_mta_feed(
                    api_key, feed_id, feed_name
                )
                ...
                logger.info(f"Feed {feed_id}: {len(vehicles)} vehicles, {len(alerts)} alerts")
...
def fetch_and_decode_mta_feed(
    ...
    """
    Fetch GTFS-RT protobuf from MTA and decode into vehicle/alert data

    Args:
        api_key: MTA API key
        feed_id: Feed ID (1, 2, 3, etc.)
    ...
    try:
        # Fetch protobuf from MTA API
        url = f"{MTA_API_BASE}?key={api_key}&feed_id={feed_id}"
        response = requests.get(url, timeout=10)
        response.raise_for_status()

        raw_bytes = len(response.content)
        logger.debug(f"Received {raw_bytes} bytes from feed {feed_id}")
        ...
        logger.info(f"Decoded {len(vehicles)} vehicles, {len(alerts)} alerts from feed {feed_id}")
        return vehicles, alerts, raw_bytes

    except requests.exceptions.RequestException as e:
        logger.error(f"HTTP error fetching feed {feed_id}: {str(e)}")
        raise
    except Exception as e:
        logger.error(f"Error decoding feed {feed_id}: {str(e)}")
        raise
