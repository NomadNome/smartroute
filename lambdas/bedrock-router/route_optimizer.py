...
from subway_graph import TRANSFER_HUB_QUALITY
...
class RouteOptimizer:
    ...
    def __init__(self, dijkstra_router, crime_data: Optional[Dict] = None):
        ...
        self.transfer_counter = {}  # Track transfer count for early-transfer tie-breaking
    ...
    def _generate_safe_route(self, origin: str, destination: str) -> Optional[Dict]:
        ...
        Transfer penalty: 5 minutes base, 2 minutes at major hubs (Tier 0)
        ...
        def weight_safe(current: str, next_station: str, current_line: str, next_line: str, travel_time: int, is_transfer: bool) -> float:
            ...
            # Tiny per-hop cost to prefer shorter paths when costs are otherwise equal
            weight += 1.0  # 1.0 per hop - breaks ties in favor of shorter paths

            # Add transfer penalty, reduced at major hubs
            if is_transfer:
                # Tier 0 hubs (major transfer points) get lower penalty (1.5 min)
                hub_quality = TRANSFER_HUB_QUALITY.get(next_station, 2)  # Default: Tier 2 penalty
                if hub_quality == 0:  # Tier 0: Major hub
                    weight += 1.5  # Minimal penalty at major hubs - prefer early transfers
                else:
                    weight += 5.0  # Standard penalty for other transfers
    ...
    def _generate_fast_route(self, origin: str, destination: str) -> Optional[Dict]:
        ...
        Total: ~8 minutes per transfer, reduced to ~4 minutes at major hubs (Tier 0)
        ...
        def weight_fast(current: str, next_station: str, current_line: str, next_line: str, travel_time: int, is_transfer: bool) -> float:
            ...
            # At Tier 0 (major hubs like Fulton, Canal): ~2 minutes (optimized transfer)
            # At other hubs: ~8 minutes (average wait + walking)
            if is_transfer:
                hub_quality = TRANSFER_HUB_QUALITY.get(next_station, 2)
                if hub_quality == 0:  # Tier 0: Major hub with short walks
                    weight += 2.0  # Minimal penalty at major hub - prefer early transfers
                else:
                    weight += 8.0  # Standard transfer penalty
    ...
    def _generate_balanced_route(self, origin: str, destination: str) -> Optional[Dict]:
        ...
        - Transfer penalty reduced at major hubs (Tier 0) for better routing decisions
        """
        ...
        def weight_balanced(
            ...
            # Transfer cost: reduced at major hubs (Tier 0)
            if is_transfer:
                hub_quality = TRANSFER_HUB_QUALITY.get(next_station, 2)
                if hub_quality == 0:  # Tier 0: Major hub
                    transfer_cost = 1.5  # Very low cost at major hubs - prefer early transfers
                else:
                    transfer_cost = 6.0  # Standard cost at other hubs
            else:
                transfer_cost = 0

            # Balanced: 50% time, 35% crime, 15% transfers
            # Add tiny per-hop cost to prefer shorter paths when costs are otherwise equal
            weight = (travel_time * 0.5) + (crime_cost * 0.35) + (transfer_cost * 0.15) + 0.5
